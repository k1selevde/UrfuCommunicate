...
router.post('/login',
    ...
    async (req, res) => {
        try {
            const errors = validationResult(req)
            if (!errors.isEmpty()) {
                return res.status(400).json({ errors: errors.array(), message: 'Некоррекиный данные при входе' })
           ...
           return res.status(200).json({data: {id: user.id, isTeacher: user.isTeacher, token}, status: 'ok'})
